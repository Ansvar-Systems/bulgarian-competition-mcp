# Bulgarian Competition MCP

**Bulgarian competition data for AI compliance tools.**
...
**Endpoint:** `https://mcp.ansvar.eu/bulgarian-competition/mcp`

| Client | How to Connect |
|--------|---------------|
| **Claude.ai** | Settings > Connectors > Add Integration > paste URL |
| **Claude Code** | `claude mcp add bulgarian-competition-mcp --transport http https://mcp.ansvar.eu/bulgarian-competition/mcp` |
...
```json
{
  "mcpServers": {
    "bulgarian-competition-mcp": {
      "type": "url",
      "url": "https://mcp.ansvar.eu/bulgarian-competition/mcp"
    }
  }
}
```
...
```bash
npx @ansvar/bulgarian-competition-mcp
```
...
```json
{
  "mcpServers": {
    "bulgarian-competition-mcp": {
      "command": "npx",
      "args": ["-y", "@ansvar/bulgarian-competition-mcp"]
    }
  }
}
```
...
| Tool | Description |
|------|-------------|
| `bg_comp_search_decisions` | Full-text search across CPC enforcement decisions (abuse of dominance, cartels, sector inquiries) under Bulgarian com... |
| `bg_comp_get_decision` | Get a specific CPC decision by case number (e.g., |
| `bg_comp_search_mergers` | Search CPC merger control decisions (concentrations). Returns merger cases with acquiring party, target, sector, and ... |
| `bg_comp_get_merger` | Get a specific CPC merger control decision by case number. |
| `bg_comp_list_sectors` | List all sectors with CPC enforcement activity, including decision counts and merger counts per sector. |
| `bg_comp_about` | Return metadata about this MCP server: version, data source, coverage, and tool list. |
...
- **CPC (Commission on Protection of Competition)** -- Official regulatory authority
...
>
> Regulatory data is sourced from official publications by CPC (Commission on Protection of Competition). However:
...
```bash
git clone https://github.com/Ansvar-Systems/bulgarian-competition-mcp
cd bulgarian-competition-mcp
npm install
npm run build
npm test
```
